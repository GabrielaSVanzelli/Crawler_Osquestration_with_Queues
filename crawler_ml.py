...
class CrawlerML():
    ...
    def _get_results(self):
        ...
        for page_n in range(1, pages+1):
            ...
            # results = response.html.xpath("//a[contains(@class, 'ui-search-link') and contains(text())]")
            results = response.html.xpath("//a[contains(@class, 'ui-search-link') and contains(@title, '.')]")
    ...
    def _get_characteristics(self, data):
        characteristics = data.html.xpath("//span[@class='andes-table__column--value']")

        try: 
            data = {
                "Área Total": characteristics[0].text if characteristics[0].text is not None else None,
                "Área Útil": characteristics[1].text if characteristics[1].text is not None else None,
                "Quartos": characteristics[2].text if characteristics[2].text is not None else None,
                "Banheiros": characteristics[3].text if characteristics[3].text is not None else None
            }
        except:
            data = None
    ...
    def _parse_data(self, data):
        ...
        data = {
            ...
            "Características": self._get_characteristics(data)
        }
